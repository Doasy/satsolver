...
if __name__ == '__main__':
    ...
    if not os.path.isfile("./limits.sh") or not (os.stat("./limits.sh").st_mode & stat.S_IXUSR):
        sys.exit("ERROR: ./limits.sh not found or without x permission.")
    ...
    for bf in benchmark_files:
        ...
        os.system("(time -p ./limits.sh %s %s) > %s 2>&1" %
                  (solver, bf, out_file))
        # Check result
